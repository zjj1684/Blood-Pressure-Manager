...
从机通过 BLE Notify 向主机推送采样数据包，每包 **17 字节**。
...
```
Byte:  [0]     [1]     [2]  [3]  [4]  [5]  [6]  [7]  ...  [16]
       ┌─────┬─────┬───────────────────────────────────────────────┐
       │Type │ Seq │  Sample 0  │  Sample 1  │...│  Sample 4  │
       │ 1B  │ 1B  │  3 Bytes   │  3 Bytes   │   │  3 Bytes   │
       └─────┴─────┴───────────────────────────────────────────────┘
             │
             ├── Type = 0x02 (数据包标识)
             └── Seq  = 0x00 ~ 0xFF (循环递增)
...
| 每包采样数 | 5 | 每包包含 5 组 PPG+ECG |
| 发包频率 | 200 包/秒 | 1000 / 5 = 200 |
| 单包大小 | 17 字节 | 1B Type + 1B Seq + 15B Data |
| 有效数据 | 16 字节/包 | 扣除 Type 后的有效载荷 |
| 带宽 | ~3400 bytes/sec | 200 × 17 = 3400 |
...
主机 (App)                           从机 (BP01)
    ...
    |--- CMD_START (0x01) ------------->|
    |                                    |
    |<-- Data Packet (Seq=0x00) --------|  ← 5 个采样点
    |<-- Data Packet (Seq=0x01) --------|  ← 5 个采样点
    |<-- Data Packet (Seq=0x02) --------|  ← 5 个采样点
    |       ... (200 包/秒) ...          |
    |                                    |
    |==== 停止阶段 =====================|
    |                                    |
